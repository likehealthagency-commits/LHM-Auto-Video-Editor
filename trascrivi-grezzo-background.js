...
async function fillTranscript(audioPath, duration, silences, words, segments){
  ...
  const holes0 = coverageHoles(words, duration, 0.35).filter(h=> h.end-h.start >= 0.6);
  const holesBefore = holes0.filter(h=> silenceFraction(h,silences) < 0.7).reduce((a,h)=>a+(h.end-h.start),0);
  ...
  for(let pass=0; pass<2; pass++){
    const holes = coverageHoles(allWords, duration, 0.35)
      .filter(h=> h.end-h.start >= 0.6 && silenceFraction(h,silences) < 0.7)
      .slice(0, 40);
